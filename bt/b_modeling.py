  ...
  def calc_output(self,input_ids,embed=None):
    ...
    with tf.variable_scope(scope, default_name="bert"):
      with tf.variable_scope("encoder"):
        ...
        #将二维mask转为3d，然后用于self-attention中惩罚<pad>的词。
...
def embedding_postprocessor(input_tensor,
  ...
  #原词向量上加了一个对词id信息的做了平滑处理的矩阵,这是论文中提到的语句向量
  if use_token_type:
    if token_type_ids is None:
...
#do_return_2d_tensor为False时返回一个三维张量：[batch_size, from_seq_length, num_attention_heads * size_per_head]
def attention_layer(from_tensor,
  ...
  # `context_layer` = [B, N, F, H],Q,K,V的计算结果
